...
public class JwtUtil {
    ...
    public UserJwtResponse extractUserClaims(String token) {
        final Claims claims = extractAllClaims(token);
        Map<String, Object> userClaims = claims.get("user", Map.class);

        if (userClaims != null) {
            UserJwtResponse userJwtResponse = new UserJwtResponse();
            userJwtResponse.setId((int) userClaims.get("id"));
            userJwtResponse.setEmail((String) userClaims.get("email"));
            userJwtResponse.setFullName((String) userClaims.get("fullName"));
